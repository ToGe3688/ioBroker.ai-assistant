...
class TriggerTool {
  ...
  */
	...
	async triggerCheck(id, stateValue) {
		for (const trigger of this.triggers) {
			if (trigger.objectId === id) {
				...
				this.adapter.log.debug(`${this.name}: Only on state value change for ${trigger.objectId} is ${trigger.onlyOnStateValueChange}`);
				...
				if (trigger.condition && trigger.condition.operator && trigger.condition.value) {
					if (this.compare(stateValue, trigger.condition.operator, trigger.condition.value) === null) {
						this.adapter.log.warn(`${this.name}: Invalid operator ${trigger.condition.operator} or value ${trigger.condition.value} for ${trigger.objectId}`);
						return false;
					}
					...
					if (trigger.condition.value == "true" || trigger.condition.value == "True") {
						this.adapter.log.debug(`${this.name}: Converting value to boolean true for ${trigger.objectId}`);
						trigger.condition.value = true;
					} else if (trigger.condition.value == "false" || trigger.condition.value == "False") {
						this.adapter.log.debug(`${this.name}: Converting value to boolean false for ${trigger.objectId}`);
						trigger.condition.value = false;
					}

					if (this.compare(stateValue, trigger.condition.operator, trigger.condition.value)) {
						this.adapter.log.info(`${this.name}: Triggered for ${trigger.objectId} with condition ${trigger.condition.operator} ${trigger.condition.value}`);
						const executionData = {
							type: "triggerWakeUpOnStateChange",
							tool: this.name,
							prompt: trigger.instruction,
							noticeToAssistant: this.adapter.I18n.translate("trigger_execute_by_condition", trigger.objectId, trigger.condition.operator, trigger.condition.value),
							result: this.adapter.I18n.translate("trigger_current_value") + stateValue
						};
						if (trigger.executeOnlyOnce) {
							this.adapter.log.debug(`${this.name}: Deleting trigger for ${trigger.objectId} with executeOnlyOnce`);
							await this.deleteTrigger(trigger.id);
						}
						this.adapter.startAssistantRequest(JSON.stringify(executionData));
					} else {
						this.adapter.log.debug(`${this.name}: Not triggered for ${trigger.objectId} with condition ${trigger.condition.operator} ${trigger.condition.value}`);
					}
				} else {
	...
	async initTriggers() {
		...
		for (const trigger of this.triggers) this.adapter.unsubscribeForeignStates(trigger.objectId);
		this.triggers = [];
		const triggers = await this.getTriggers();
		for (const trigger of triggers) {
			try {
				this.createTrigger(trigger.id, trigger.objectId, trigger.condition, trigger.instruction, trigger.onlyOnStateValueChange, trigger.executeOnlyOnce);
			} catch (e) {
			...
			if (id.includes("Triggers.")) {
				...
				try {
					...
					if (triggerData && triggerData.objectId && triggerData.instruction) {
						this.adapter.log.debug(`${this.name}: Found trigger for ${id} with objectId ${triggerData.objectId} and instruction ${triggerData.instruction}`);
						triggers.push({ id: id, objectId: triggerData.objectId, condition: triggerData.condition, instruction: triggerData.instruction, onlyOnStateValueChange: triggerData.onlyOnStateValueChange, executeOnlyOnce: triggerData.executeOnlyOnce });
					}
				} catch (e) {
	...
	async createTrigger(triggerId, objectId, condition, instruction, onlyOnStateValueChange, executeOnlyOnce) {
		if (!triggerId || !objectId || !instruction) return this.adapter.log.warn(`Cannot create trigger for ${triggerId} with objectId ${objectId} and instruction ${instruction}`);
		this.adapter.log.debug(`${this.name}: Creating trigger for ${triggerId} with objectId ${objectId} and instruction ${instruction}`);
		this.adapter.log.debug(`${this.name}: Trigger Data: ObjectId` + objectId + ` Condition: ` + condition + ` Instruction: ` + instruction + ` OnlyOnStateValueChange: ` + onlyOnStateValueChange + ` ExecuteOnlyOnce: ` + executeOnlyOnce);
		this.adapter.log.debug(`${this.name}: Subscribing to state changes for ${objectId}`);
		this.adapter.subscribeForeignStates(objectId);
		this.triggers.push({ id: triggerId, objectId: objectId, condition: condition, instruction: instruction, onlyOnStateValueChange: onlyOnStateValueChange, executeOnlyOnce: executeOnlyOnce });
	...
	async request(prompt) {
		...
		const modelResponse = await this.adapter.startModelRequest([{ role: "user", content: await this.constructToolMessage(prompt) }], this.system_prompt.replace(/(\r\n|\n|\t|\r)/gm, ""), this.max_tokens, this.temperature);
	...
	async handleResponse(modelResponse) {
		...
		try {
			...
			if (responseData.createTriggers && Array.isArray(responseData.createTriggers) && responseData.createTriggers.length > 0) {
				for (const trigger of responseData.createTriggers) {
					...
					const triggerString = JSON.stringify({ objectId: trigger.objectId, condition: trigger.condition, instruction: trigger.instruction, onlyOnStateValueChange: trigger.onlyOnStateValueChange, executeOnlyOnce: trigger.executeOnlyOnce });
					await this.adapter.setStateAsync(id, triggerString);
					this.createTrigger(id, trigger.objectId, trigger.condition, trigger.instruction, trigger.onlyOnStateValueChange, trigger.executeOnlyOnce);
					createdTriggers.push({ objectId: trigger.objectId, condition: trigger.condition, instruction: trigger.instruction, onlyOnStateValueChange: trigger.onlyOnStateValueChange, executeOnlyOnce: trigger.executeOnlyOnce });
				}
			}
			if (responseData.deleteTriggers && Array.isArray(responseData.deleteTriggers) && responseData.deleteTriggers.length > 0) {
