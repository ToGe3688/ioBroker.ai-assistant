  ...
  */
	...
	async handleResponse(modelResponse) {
		...
		try {
			...
			if (responseData["set-states"] && Array.isArray(responseData["set-states"]) && responseData["set-states"].length > 0) {
				result.setStates = await this.setStatesForDatapoints(responseData["set-states"]);
			}

			if (responseData["get-states"] && Array.isArray(responseData["get-states"]) && responseData["get-states"].length > 0) {
