...
class SchedulerTool {
  ...
  */
	constructor(adapter) {
		...
		this.adapter.log.debug(`Created new SetTimeHooksTool`);
	...
	async initCronJobs() {
		...
		for (const job in this.cronJobs) schedule.cancelJob(job);
		this.cronJobs = [];
			...
			if (id.includes("Cronjobs.")) {
				...
				try {
					...
					if (cronData && cronData.cron && cronData.instruction) {
						this.adapter.log.debug(`${this.name}: Found cron job for ${id} with cron ${cronData.cron} and instruction ${cronData.instruction}`);
	...
	createCronJob(objectId, cronExpression, instruction) {
		if (!objectId || !cronExpression || !instruction) return this.adapter.log.warn(`Cannot create cron job for ${objectId} with expression ${cronExpression} and instruction ${instruction}`);
		this.adapter.log.debug(`${this.name}: Creating cronjob for ${objectId} with expression ${cronExpression} and instruction ${instruction}`);
	...
	async handleResponse(modelResponse) {
		...
		try {
			...
			if (responseData.createCronjobs && Array.isArray(responseData.createCronjobs) && responseData.createCronjobs.length > 0) {
				for (const cronjob of responseData.createCronjobs) {
					...
					const cronjobString = JSON.stringify({ cron: cronjob.cronExpression, instruction: cronjob.instruction });
			...
			if (responseData.createTimeouts && Array.isArray(responseData.createTimeouts) && responseData.createTimeouts.length > 0) {
				for (const timeout of responseData.createTimeouts) {
					setTimeout(() => {
						const timeoutExecutionData = {
							...
							noticeToAssistant: this.adapter.I18n.translate("scheduler_execute_timeout", timeout.timeoutSeconds),
							result: this.adapter.I18n.translate("scheduler_current_time") + new Date().toLocaleString()
						};
						this.adapter.startAssistantRequest(JSON.stringify(timeoutExecutionData));
					}, timeout.timeoutSeconds * 1000, timeout);
					createdTimeouts.push({ timeoutSeconds: timeout.timeoutSeconds, instruction: timeout.instruction });
				}
			}
			if (responseData.deleteCronjobs && Array.isArray(responseData.deleteCronjobs) && responseData.deleteCronjobs.length > 0) {
			...
			const toolResponse = {
				...
				result: {createdCronjobs: createdCronjobs, deletedCronjobs: deletedCronjobs, createdTimeouts: createdTimeouts}
			};
			return toolResponse;
		} catch (error) {
