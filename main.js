...
class AiAssistant extends utils.Adapter {
	...
	async onReady() {
		...
		this.log.info("Starting adapter with provider: " + this.provider.name + " and model: " + this.config.assistant_model);
	...
	async startAssistantRequest(text, tries = 0, try_only_once = false, functionResponse = false) {
		...
		if (this.provider) {
			...
			const modelResponse = await this.startModelRequest(messages, await this.buildSystemPrompt(), this.config.max_tokens, this.config.temperature);
			...
			if (!requestCompleted) {
				if (!this.config.retry_delay) { this.config.retry_delay = 15; }
				if (!this.config.max_retries) { this.config.max_retries = 3; }
				...
				await this.addMessagePairToHistory(text, modelResponse.text, modelResponse.tokens_input, modelResponse.tokens_output, modelResponse.model);
	...
	async getCustomFunctionsPrompt() {
		...
		if (this.config.available_functions && this.config.available_functions.length > 0) {
			for (const customFunction of this.config.available_functions) {
				customFunctionPrompt = customFunctionPrompt + "{\"name\":\"" + customFunction.name + "\", \"description\":\"" + customFunction.description + "\"},\n";
			}
		}
	...
	async startModelRequest(messages, system_prompt = null, max_tokens = 2000, temperature = 0.6) {
		...
		if (this.provider) {
			...
			if (modelResponse.error) {
				...
				await this.setStateAsync("Assistant.text_response", { val: "Error: " + modelResponse.error, ack: true });
			} else {
...
FunctionInstruction: ${responseData.functionTextInstructionString}
			...
			if (responseData.functionCall) {
				...
				if (!responseData.functionTextInstructionString) {
					this.log.warn("No function instruction provided in assistant response data. Called function: " + responseData.functionCall);
				}
				...
				const functionResponse = await this.tryToExecuteFunctionCall(responseData.functionCall, responseData.functionTextInstructionString);
				if (functionResponse) {
					if (this.config.assistant_debug_output) {
						const debugOutput = `
...
FunctionResultData: ${JSON.stringify(functionResponse.result)}
	...
	async tryToExecuteFunctionCall(functionCall, functionInstruction) {
		...
		if (functionCall == "deleteHistory") {
			...
			await this.setStateAsync("Assistant.text_response", { val: I18n.translate("assistant_function_delete_history_success"), ack: true });
			setTimeout(async () => {
				await this.clearHistory();
			}, 3000);
			return null;
		...
		return new Promise((resolve) => {
			const checkInterval = setInterval(async () => {
				...
				if (newResultTimestamp > oldResultTimestamp) {
					this.log.info("Result received from function: " + customFunction.name + " Result: " + newResultState.val);
					clearInterval(checkInterval);
					resolve(newResultState.val);
				}

				if (tries >= 60) {
					this.log.warn("No result received from function: " + customFunction.name + " on datapoint " + customFunction.objId_result + " after 60 seconds");
					clearInterval(checkInterval);
	...
	async onMessage(obj) {
		...
		if (typeof obj === "object" && obj.message) {
			...
			if (obj.command === "importFromEnums") {
				...
				if (obj.callback) this.sendTo(obj.from, obj.command, { native: { available_endpoints: objects } }, obj.callback);
			}
